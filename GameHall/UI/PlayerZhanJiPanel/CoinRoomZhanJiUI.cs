...
class CoinRoomZhanJiUI : MonoBehaviour
{
    /// <summary>
    /// 场次.
    /// </summary>
    ...
    {
        Debug.Log("Unity: Coin.OnClickHuiGuBt -> IndexVal: " + IndexVal);
    }
    /// <summary>
    /// 点击战绩列表单元按键.
    /// </summary>
    ...
    {
        Debug.Log("Unity: Coin.OnClickZhanJiUIBt -> IndexVal: " + IndexVal);
    }
    /// <summary>
    /// 显示战绩数据.
    /// </summary>
    public void ShowZhanJiInfo(object zjDt)
    {
        ...
        for (int i = 0; i < dataList.Count; i++)
        {
            if (i == 2)
            {
                switch (System.Convert.ToInt32(dataList[i]))
                {
                    case 0:
                        changCiVal = "金币房";
                        break;
