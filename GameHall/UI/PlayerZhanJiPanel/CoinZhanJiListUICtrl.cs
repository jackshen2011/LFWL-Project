...
/// 金币房间战绩列表控制.
/// </summary>
class CoinZhanJiListUICtrl : MonoBehaviour
{
    /// <summary>
    /// 总对局.
    /// </summary>
    ...
    {
        object[] args = MainRoot._gUIModule.pUnModalUIControl.CoinRoomZhanJiArgs;
        ...
        ShengLvTx.gameObject.SetActive(isHaveBiSai);
        if (!isHaveBiSai)
        {
            return;
        }
        ...
        int lianShengVal = isTestZhanJi == true ? 10 : (int)args[6];
        float shengLvVal = (float)lianShengVal / zongDuiJuVal;
        shengLvVal = (float)((int)(shengLvVal * 10000)) / 100; //转化为小数点后2位数.
        ...
        {
            GameObject obj = (GameObject)GameObject.Instantiate(Resources.Load("Prefab/PlayerZhanJiPanel/PlayerCoinZhanJiUI"),
                                                                ZhanJiCardListPtr, false);
            ZhanJiTrAy[i] = obj.transform;
            obj.name = "PlayerCoinZhanJiUI" + i;
        }

        Vector3 startPos = new Vector3(0f, (cardHeight * 0.5f) - (((RectTransform)ZhanJiTrAy[0]).sizeDelta.y * 0.5f), 0f);
        CoinRoomZhanJiUI zhanJiUI = null;
        for (int i = 0; i < ZhanJiCount; i++)
        {
            offsetPos.y = -i * OffsetPosY;
            ZhanJiTrAy[i].localPosition = startPos + offsetPos;

            zhanJiUI = ZhanJiTrAy[i].GetComponent<CoinRoomZhanJiUI>();
